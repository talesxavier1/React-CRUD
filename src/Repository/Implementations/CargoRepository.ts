...
export default class CargoRepository implements ICargoRepository {
    ...
    countPositions = async (userToken: string, query?: string) => {
        let url = `${import.meta.env.VITE_REACT_APP_API}/position/countPositions{0}`.replace("{0}", query ? `?query=${query}` : "");
    ...
    getPositions = async (userToken: string, skip: number, take: number, query?: string) => {
        let url = `${import.meta.env.VITE_REACT_APP_API}/position/getPositions?skip={0}&take={1}{2}`
